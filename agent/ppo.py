...
class PPO:
    ...
    def rollout(self, problem, val_m, batch, do_sample = False, show_bar = False):
        ...
        # Also expand dist matrix if present (for OSM-based problems)
        # dist shape: [bs, gs, gs] -> [bs, 1, gs, gs] -> [bs, val_m, gs, gs]
        if 'dist' in batch and batch['dist'] is not None:
            batch['dist'] = batch['dist'].unsqueeze(1).repeat(1, val_m, 1, 1)
        ...
        # Reshape coordinates: [bs, val_m, gs, dim] -> [bs*val_m, gs, dim]
        batch['coordinates'] =  batch['coordinates'].view(-1, gs, dim)
        
        # Reshape dist matrix if present: [bs, val_m, gs, gs] -> [bs*val_m, gs, gs]
        if 'dist' in batch and batch['dist'] is not None:
        ...
        for t in tqdm(range(self.opts.T_max), disable = self.opts.no_progress_bar or not show_bar, desc = 'rollout', bar_format='{l_bar}{bar:20}{r_bar}{bar:-20b}'):       
            ...
            # Update best solutions when we find better ones
            # After step(), obj becomes 2D: [current_cost, best_so_far_cost]
            current_obj = obj[:,-1] if obj.dim() > 1 else obj  # Handle both 1D and 2D cases
            improved_mask = current_obj < best_obj
            best_solutions[improved_mask] = solutions[improved_mask]
            best_obj[improved_mask] = current_obj[improved_mask]
...
def train(rank, problem, agent, val_dataset, tb_logger):
    ...
    # ========================================================================
    # Pre-load training dataset ONCE for pdtsp_osm (instead of every epoch)
    # ========================================================================
    preloaded_training_dataset = None
    if problem.NAME == 'pdtsp_osm' and opts.train_dataset is not None:
        # Load the entire pre-generated dataset once before all epochs
        if rank == 0:
            print('\n' + '='*60)
        ...
        preloaded_training_dataset = problem.make_dataset(
            ...
            num_samples=None,  # Load all data from file
        ...
        if rank == 0:
            print(f"✅ Loaded {len(preloaded_training_dataset)} training instances from: {opts.train_dataset}")
            print(f"   This dataset will be reused across all {opts.epoch_end - opts.epoch_start} epochs")
            print('='*60 + '\n')
